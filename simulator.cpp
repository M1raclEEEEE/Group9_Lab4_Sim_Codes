...
void render(void) {
    ...
    for (char c : "VGA Display - Lab 4 State Machine") {
    ...
    for (char c : "Press 's' to switch states, 'a' to reset") {
        glutBitmapCharacter(GLUT_BITMAP_9_BY_15, c);
    }

    glRasterPos2f(-0.9f, -0.85f);
    for (char c : "LED Indicators: S(System) B(Button) 1-3(States)") {
        glutBitmapCharacter(GLUT_BITMAP_9_BY_15, c);
    }
    ...
    for(int i = 0; i < 5; i++){
        ...
        if (i == 0) {
            ...
            glutBitmapCharacter(GLUT_BITMAP_9_BY_15, '1' + (i-2));
        }
    }
...
void graphics_loop(int argc, char** argv) {
    ...
    glutCreateWindow("VGA Simulator - Lab 4 State Machine");
